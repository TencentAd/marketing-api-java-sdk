...
public class AddLeadsAds {
  ...
  /**
   * 图片文件路径，450规格要求：800x450, <300K, png/jpg
   */
  public String IMAGE_PATH = "YOUR AD IMAGE PATH";
  /**
   * Profile图片文件路径，200x200, <300K, png/jpg
   */
  ...
  public Long addLeadsAds() throws Exception {
    ...
    // 第四步，创建创意
    Long adcreativeId = addAdcreative(ADCREATIVE_TEMPLATE_ID, AD_URL, PromotedObjectType.LEAD_AD,
        AD_TITLE, AD_DESCRIPTION, campaignId, Arrays.asList(Long.valueOf(imageId)), siteSet, profileId);
    // 第五步，创建广告
  ...
  protected Long addCampaign(CampaignType campaignType, PromotedObjectType promotedObjectType)
    ...
    campaignsAddRequest
        .setCampaignName("SDK sample campaign" + UUID.randomUUID().toString().substring(0, 6));
    ...
    CampaignsAddResponseData responseData = tencentAds.campaigns()
        .campaignsAdd(campaignsAddRequest);
   ...
   * @throws ApiException
   */
  protected Long addAdgroups(Long campaignId, PromotedObjectType promotedObjectType,
      List<String> siteSet)
      throws ApiException {
    ...
    //全天投放
    String timeSeries = "11111111111111111111111111111111111111111111111111111111"
    ...
    targeting.age(Arrays.asList(new AgeStruct().max(45L).min(23L))).gender(Arrays.asList("MALE"))
        .geoLocation(new GeoLocations().locationTypes(Arrays.asList("LIVE_IN"))
  ...
  protected String addImage(String imageFile) throws Exception {
    ImagesAddResponseData responseData = tencentAds.images()
        .imagesAdd(ACCOUNT_ID, "UPLOAD_TYPE_FILE", SignatureUtils.getMD5Checksum(imageFile),
            new File(imageFile), null, null);
    if (responseData != null) {
   ...
   * @throws ApiException
   */
  protected Long addAdcreative(Long adcreativeTemplateId, String pageUrl,
      PromotedObjectType promotedObjectType, String creativeElementsTitle,
      String creativeElementsDesc, Long campaignId, List<Long> imageIdList, List<String> siteSet,
      Long profileId) throws ApiException {
    ...
    data.setAdcreativeElements(
        new AdcreativeCreativeElementsMp().imageList(imageIdList).title(creativeElementsTitle)
