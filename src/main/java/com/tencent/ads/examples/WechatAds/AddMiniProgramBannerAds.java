...
public class AddMiniProgramBannerAds {
  ...
  public Long addMiniProgramBannerAds() throws Exception {
    ...
    Long adcreativeId = addAdcreative(ADCREATIVE_TEMPLATE_ID, campaignId,
        promotedObjectType, MINI_GAME_ID, siteSet, imageId, MINI_GAME_PATH);
  ...
  private Long addAdcreative(Long adcreativeTemplateId, Long campaignId,
      PromotedObjectType promotedObjectType, String promotedObjectId, List<String> siteSet,
      String imageId,
      String miniProgramPath) throws ApiException {
    ...
    AdcreativesAddResponseData responseData = tencentAds.adcreatives().adcreativesAdd(
        new AdcreativesAddRequest().accountId(ACCOUNT_ID).campaignId(campaignId)
            ...
            .adcreativeElements(
                new AdcreativeCreativeElementsMp().imageList(Arrays.asList(Long.valueOf(imageId))))
            .promotedObjectType(promotedObjectType).pageType(
            pageType).pageSpec(new PageSpec().miniProgramSpec(
            new AdcreativeMiniProgramSpec().miniProgramId(promotedObjectId)
                .miniProgramPath(miniProgramPath))).siteSet(siteSet)
            .promotedObjectId(promotedObjectId));
