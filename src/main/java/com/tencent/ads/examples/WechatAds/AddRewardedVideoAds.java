...
public class AddRewardedVideoAds {
   ...
   * YOUR AD VIDEO_PATH
   */
  public static String VIDEO_PATH = "YOUR AD VIDEO PATH"; // 广告主视频文件路径，560规格要求：750x1334, 6-30秒, <7168K, mp4
  /**
   * YOUR AD TEXT
   */
   ...
   */
  public static String LOGO_IMAGE_PATH = "YOUR LOGO IMAGE PATH"; // 广告品牌LOGO图文件路径，560规格要求：300x300, <50K, png/jpg
  /**
   * YOUR LOGO CORP NAME
   */
  ...
  public Long addRewardedVideoAds() throws Exception {
    ...
    // 第五步，创建创意
    Long adcreativeId = addAdcreative(ADCREATIVE_TEMPLATE_ID, campaignId, promotedObjectType,
        promotedObjectId, siteSet, videoId, AD_TITLE, imageId, LOGO_CORP_NAME);
  ...
  private Long addVideo(String videoFile) throws Exception {
    ...
    VideosAddResponseData responseData = tencentAds.videos()
        .videosAdd(ACCOUNT_ID, new File(videoFile), imageSignature, null, null);
  ...
  private Long addAdcreative(Long adcreativeTemplateId, Long campaignId,
      PromotedObjectType promotedObjectType, String promotedObjectId, List<String> siteSet,
      Long videoId, String adTitle, String logoImageId, String logoCorpName) throws ApiException {
    ...
    AdcreativesAddResponseData responseData = tencentAds.adcreatives().adcreativesAdd(
        new AdcreativesAddRequest().accountId(ACCOUNT_ID).campaignId(campaignId)
            ...
            .adcreativeElements(
                new AdcreativeCreativeElementsMp().title(adTitle).video(videoId)
                    .corporate(new AdcreativeCorporate().corporateName(logoCorpName)
                        .corporateImg(logoImageId))).promotedObjectType(promotedObjectType)
            .pageType(
                pageType).siteSet(siteSet)
