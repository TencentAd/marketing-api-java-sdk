...
public class RapidAdsContainer extends ApiContainer {
  ...
  public RapidAddAdsResponseData rapidAddAds(RapidAddAdsRequestData rapidAddAdsRequest) throws TencentAdsResponseException {
    ...
    for (int i = 0; i < ATTEMPT_TIMES; i++) {
      try {
        rapidAddAdsResponse.campaignsAddResponseData = campaignsApi.campaignsAdd(rapidAddAdsRequest.campaignsAddRequest);
    ...
    try {
      BatchAddResult batchAddResult = batchAdd(accountId, campaignId, rapidAddAdsRequest, rapidAddAdsResponse);
  ...
  protected void batchUpdate(Long accountId, Long campaignId, Long adgroupId, Long adId, AdStatus campaignStatus, AdStatus adgroupStatus, AdStatus adStatus) throws TencentAdsResponseException {
    ...
    List<Class<?>> requestClasses = new ArrayList<>(Arrays.asList(CampaignsUpdateRequest.class, AdgroupsUpdateRequest.class, AdsUpdateRequest.class));
    List<Class<?>> responseClasses = new ArrayList<>(Arrays.asList(CampaignsUpdateResponse.class, AdgroupsUpdateResponse.class, AdsUpdateResponse.class));

    List<BatchRequestSpecStruct> batchRequestSpecs = new ArrayList<>();
    CampaignsUpdateRequest campaignsUpdateRequest = buildCampaignUpdateRequest(accountId, campaignId, campaignStatus);
    ...
    AdgroupsUpdateRequest adgroupsUpdateRequest = buildAdgroupUpdateRequest(accountId, adgroupId, adgroupStatus);
    ...
    for (int attemptTime = 0; attemptTime < ATTEMPT_TIMES; attemptTime++) {
      try {
        BatchRequestsAddResponseData batchRequestsAddResponse = batchRequestsApi.batchRequestsAdd(batchRequestsAddRequest);
...
//    String version = TencentAds.API_VERSION;
    ...
    for (int i = 0; i < ATTEMPT_TIMES; i++) {
      ...
      List<BatchRequestSpecStruct> batchRequestSpecs = new ArrayList<>();
      boolean needAdgroup = batchAddResult.adgroupId == null, needAdcreative = batchAddResult.adcreativeId == null;
      if (needAdgroup) {
      ...
      if (needAdcreative) {
        String adcreativeAddFormData = buildFormData(rapidAddAdsRequest.adcreativesAddRequest.toString());
      ...
      try {
        ...
        if (needAdgroup) {
          AdgroupsAddResponse adgroupsAddResponse = parseResponse(adgroupsApiResponse.getBody(), AdgroupsAddResponse.class);
        ...
        if (needAdcreative) {
          AdcreativesAddResponse adcreativesAddResponse = parseResponse(adcreativeApiResponse.getBody(), AdcreativesAddResponse.class);
  ...
  protected Long getFailedAdgroup(Long accountId, Long campaignId) {
    Long adgroupId = null;
    List<FilteringStruct> filtering = Collections.singletonList(
        new FilteringStruct().field("campaign_id").operator("EQUALS").values(Collections.singletonList(String.valueOf(campaignId)))
    );
    List<String> fields = Collections.singletonList("adgroup_id");
    for (int i = 0; i < ATTEMPT_TIMES; i++) {
      try {
        AdgroupsGetResponseData adgroupsGetResponse = adgroupsApi.adgroupsGet(accountId, filtering, null, null, null, fields);
  ...
  protected Long getFailedAdcreative(Long accountId, Long campaignId) {
    ...
    for (int i = 0; i < ATTEMPT_TIMES; i++) {
      try {
        AdcreativesGetResponseData adcreativesGetResponseData = adcreativesApi.adcreativesGet(accountId, filtering, null, null, null, fields);
        if (adcreativesGetResponseData.getList() != null && adcreativesGetResponseData.getList().size() == 1)
  ...
  protected Long getFailedAd(Long accountId, Long campaignId) {
    ...
    for (int i = 0; i < ATTEMPT_TIMES; i++) {
      try {
        AdsGetResponseData adsGetResponseData = adApi.adsGet(accountId, filtering, null, null, null, fields);
  ...
  public static class RapidAddAdsRequestData {
    ...
    public RapidAddAdsRequestData(CampaignsAddRequest campaignsAddRequest, AdgroupsAddRequest adgroupsAddRequest,
        AdcreativesAddRequest adcreativesAddRequest, AdsAddRequest adAddRequest) {
